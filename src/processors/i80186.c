...
// 16-bit register indices as encoded in 80186 opcodes
enum { R_AX = 0, R_CX, R_DX, R_BX, R_SP, R_BP, R_SI, R_DI };
// Segment register indices as encoded in 80186 opcodes
enum { SEG_ES = 0, SEG_CS, SEG_SS, SEG_DS };

typedef struct I80186CPU {
...
static inline uint8_t mem_read8(I80186CPU *cpu, uint16_t seg, uint16_t off) {
    return cpu->memory[linear(seg, off)];
}

static inline void mem_write8(I80186CPU *cpu, uint16_t seg, uint16_t off, uint8_t val) {
    cpu->memory[linear(seg, off)] = val;
}

static inline uint16_t mem_read16(I80186CPU *cpu, uint16_t seg, uint16_t off) {
    return (uint16_t)(mem_read8(cpu, seg, off) | ((uint16_t)mem_read8(cpu, seg, (uint16_t)(off + 1)) << 8));
}

static inline void mem_write16(I80186CPU *cpu, uint16_t seg, uint16_t off, uint16_t val) {
    mem_write8(cpu, seg, off, (uint8_t)(val & 0xFF));
    mem_write8(cpu, seg, (uint16_t)(off + 1), (uint8_t)(val >> 8));
}

static inline uint8_t fetch8(I80186CPU *cpu) {
    uint8_t v = mem_read8(cpu, cpu->segs[SEG_CS], cpu->ip);
    cpu->ip++;
    return v;
}

static inline uint16_t fetch16(I80186CPU *cpu) {
...
static inline uint8_t get_reg8(I80186CPU *cpu, uint8_t r) {
    if (r < 4) return (uint8_t)(cpu->regs[r] & 0xFF);
    return (uint8_t)(cpu->regs[r - 4] >> 8);
}

static inline void set_reg8(I80186CPU *cpu, uint8_t r, uint8_t val) {
...
static RMOperand decode_modrm(I80186CPU *cpu, uint8_t modrm, int seg_ovr) {
    RMOperand op;
...
static inline uint8_t read_rm8(I80186CPU *cpu, const RMOperand *op) {
    if (op->is_reg) return get_reg8(cpu, op->reg);
    return mem_read8(cpu, op->seg, op->off);
}

static inline void write_rm8(I80186CPU *cpu, const RMOperand *op, uint8_t val) {
    if (op->is_reg) set_reg8(cpu, op->reg, val);
    else mem_write8(cpu, op->seg, op->off, val);
}

static inline uint16_t read_rm16(I80186CPU *cpu, const RMOperand *op) {
    if (op->is_reg) return cpu->regs[op->reg];
    return mem_read16(cpu, op->seg, op->off);
}

static inline void write_rm16(I80186CPU *cpu, const RMOperand *op, uint16_t val) {
...
static inline void update_flags_zsp8(I80186CPU *cpu, uint8_t res) {
    cpu->flags_z = (res == 0) ? 1 : 0;
    cpu->flags_s = (res & 0x80) ? 1 : 0;
    cpu->flags_p = calculate_parity(res);
}

static inline void update_flags_zsp16(I80186CPU *cpu, uint16_t res) {
    cpu->flags_z = (res == 0) ? 1 : 0;
    cpu->flags_s = (res & 0x8000) ? 1 : 0;
    cpu->flags_p = calculate_parity((uint8_t)(res & 0xFF));
}

static uint8_t add8(I80186CPU *cpu, uint8_t a, uint8_t b, uint8_t carry_in) {
...
static uint16_t add16(I80186CPU *cpu, uint16_t a, uint16_t b, uint8_t carry_in) {
...
static uint8_t sub8(I80186CPU *cpu, uint8_t a, uint8_t b, uint8_t borrow_in) {
...
static uint16_t sub16(I80186CPU *cpu, uint16_t a, uint16_t b, uint8_t borrow_in) {
...
static inline void flags_logic8(I80186CPU *cpu, uint8_t res) {
    cpu->flags_c = 0;
    cpu->flags_o = 0;
    cpu->flags_a = 0;
    update_flags_zsp8(cpu, res);
...
static inline void flags_logic16(I80186CPU *cpu, uint16_t res) {
    cpu->flags_c = 0;
    cpu->flags_o = 0;
    cpu->flags_a = 0;
    update_flags_zsp16(cpu, res);
...
static uint16_t get_flags_word(I80186CPU *cpu) {
    uint16_t f = 0xF002;
...
static void set_flags_word(I80186CPU *cpu, uint16_t f) {
...
static uint8_t alu8(I80186CPU *cpu, uint8_t idx, uint8_t a, uint8_t b) {
    switch (idx) {
...
static uint16_t alu16(I80186CPU *cpu, uint8_t idx, uint16_t a, uint16_t b) {
    switch (idx) {
...
static int check_cond(I80186CPU *cpu, uint8_t cond) {
    int r = 0;
    switch (cond >> 1) {
...
static inline void push16(I80186CPU *cpu, uint16_t val) {
    cpu->regs[R_SP] = (uint16_t)(cpu->regs[R_SP] - 2);
    mem_write16(cpu, cpu->segs[SEG_SS], cpu->regs[R_SP], val);
}

static inline uint16_t pop16(I80186CPU *cpu) {
...
static uint8_t shift8(I80186CPU *cpu, uint8_t idx, uint8_t val, uint8_t count) {
    count &= 0x1F;
...
static uint16_t shift16(I80186CPU *cpu, uint8_t idx, uint16_t val, uint8_t count) {
    count &= 0x1F;
    if (count == 0) return val;
    for (uint8_t i = 0; i < count; ++i) {
...
void* i80186_create(void) {
    return calloc(1, sizeof(I80186CPU));
}
...
int i80186_step(void *context) {
    ...
    switch (op) {
        ...
        case 0x61:
            ...
            (void)pop16(cpu); // SP value discarded
        ...
        // --- 186: BOUND r16, m16&16 (violation halts, no interrupt vector attached) ---
        case 0x62: {
            uint8_t modrm = fetch8(cpu);
            uint8_t reg = (uint8_t)((modrm >> 3) & 7);
            RMOperand rm = decode_modrm(cpu, modrm, seg_ovr);
            if (!rm.is_reg) {
                int16_t idx_val = (int16_t)cpu->regs[reg];
                int16_t lo = (int16_t)mem_read16(cpu, rm.seg, rm.off);
                int16_t hi = (int16_t)mem_read16(cpu, rm.seg, (uint16_t)(rm.off + 2));
                if (idx_val < lo || idx_val > hi) { cpu->halted = 1; return -2; }
            }
            break;
        }

        // --- 186: PUSH imm16 / imm8 (sign-extended) ---
        case 0x68: push16(cpu, fetch16(cpu)); break;
        case 0x6A: push16(cpu, (uint16_t)(int16_t)(int8_t)fetch8(cpu)); break;

        // --- 186: IMUL r16, r/m16, imm16/imm8 ---
        case 0x69: case 0x6B: {
            uint8_t modrm = fetch8(cpu);
            uint8_t reg = (uint8_t)((modrm >> 3) & 7);
            RMOperand rm = decode_modrm(cpu, modrm, seg_ovr);
            int16_t src = (int16_t)read_rm16(cpu, &rm);
            int16_t imm = (op == 0x69) ? (int16_t)fetch16(cpu) : (int16_t)(int8_t)fetch8(cpu);
            int32_t prod = (int32_t)src * imm;
            cpu->regs[reg] = (uint16_t)(prod & 0xFFFF);
            cpu->flags_c = cpu->flags_o = (prod != (int16_t)(prod & 0xFFFF)) ? 1 : 0;
            break;
        }

        // --- 186: INS / OUTS (no port hardware: IN reads 0, OUT discards) ---
        case 0x6C: case 0x6D: case 0x6E: case 0x6F: {
            ...
            for (;;) {
                ...
                if (op < 0x6E) { // INS: port DX -> ES:DI
                    ...
                    if (wide) (void)mem_read16(cpu, src_seg, cpu->regs[R_SI]);
                    else (void)mem_read8(cpu, src_seg, cpu->regs[R_SI]);
        ...
        // --- 186: shifts / rotates by imm8 ---
        case 0xC0: {
            uint8_t modrm = fetch8(cpu);
            RMOperand rm = decode_modrm(cpu, modrm, seg_ovr);
            uint8_t val = read_rm8(cpu, &rm);
            uint8_t count = fetch8(cpu);
            write_rm8(cpu, &rm, shift8(cpu, (uint8_t)((modrm >> 3) & 7), val, count));
            break;
        }
        case 0xC1: {
            uint8_t modrm = fetch8(cpu);
            ...
            uint8_t count = fetch8(cpu);
            write_rm16(cpu, &rm, shift16(cpu, (uint8_t)((modrm >> 3) & 7), val, count));
            break;
        }
        ...
        case 0xC8: {
            ...
            uint16_t frame_ptr;
            push16(cpu, cpu->regs[R_BP]);
            frame_ptr = cpu->regs[R_SP];
            if (level > 0) {
                for (uint8_t i = 1; i < level; ++i) {
                    cpu->regs[R_BP] = (uint16_t)(cpu->regs[R_BP] - 2);
                    push16(cpu, mem_read16(cpu, cpu->segs[SEG_SS], cpu->regs[R_BP]));
                }
                push16(cpu, frame_ptr);
            }
            cpu->regs[R_BP] = frame_ptr;
            cpu->regs[R_SP] = (uint16_t)(cpu->regs[R_SP] - alloc);
            break;
        }
        case 0xC9:
...
void i80186_print_state(void *context) {
    ...
    printf("Intel 80186 CPU State (Ticks: %u):\n", cpu->ticks);
...
static uint8_t dpeek8(I80186CPU *cpu, uint16_t *off) {
    uint8_t v = mem_read8(cpu, cpu->segs[SEG_CS], *off);
    (*off)++;
    return v;
}

static uint16_t dpeek16(I80186CPU *cpu, uint16_t *off) {
...
static void dasm_rm(I80186CPU *cpu, uint16_t *off, uint8_t modrm, int wide, char *out, size_t n) {
    uint8_t mod = (uint8_t)(modrm >> 6);
    uint8_t rm = (uint8_t)(modrm & 7);
...
void i80186_get_disassembly(void *context, char *buf, size_t buf_len) {
    ...
    switch (op) {
        ...
        case 0x69: case 0x6B: {
            ...
            dasm_rm(cpu, &off, modrm, 1, rm_str, sizeof(rm_str));
            if (op == 0x69)
                snprintf(buf, buf_len, "%sIMUL  %s, %s, 0x%04X", prefix,
                         reg16_names[(modrm >> 3) & 7], rm_str, dpeek16(cpu, &off));
            else
                snprintf(buf, buf_len, "%sIMUL  %s, %s, 0x%04X", prefix,
                         reg16_names[(modrm >> 3) & 7], rm_str, (uint16_t)(int16_t)(int8_t)dpeek8(cpu, &off));
            return;
        }
        ...
        case 0xC8: {
            uint16_t alloc = dpeek16(cpu, &off);
            snprintf(buf, buf_len, "ENTER 0x%04X, %u", alloc, dpeek8(cpu, &off));
            return;
        }
