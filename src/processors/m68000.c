...
// Motorola 68000 CPU core (practical subset).
...
void* m68000_create(void) {
    M68000CPU *cpu = (M68000CPU*)calloc(1, sizeof(M68000CPU));
    return cpu;
}
...
int m68000_init(void *context) {
    if (!context) return -1;
    M68000CPU *cpu = (M68000CPU*)context;
...
static void set_nz(M68000CPU *cpu, uint32_t val, int size) {
    val &= size_mask(size);
...
static void set_logic_flags(M68000CPU *cpu, uint32_t val, int size) {
    set_nz(cpu, val, size);
    cpu->sr &= (uint16_t)~(FLAG_V | FLAG_C);
}

static uint32_t do_add(M68000CPU *cpu, uint32_t dst, uint32_t src, int size) {
...
static uint32_t do_sub(M68000CPU *cpu, uint32_t dst, uint32_t src, int size, int set_x) {
    uint32_t mask = size_mask(size), msb = size_msb(size);
    src &= mask; dst &= mask;
    uint32_t r = (dst - src) & mask;
...
// type: 0 = AS, 1 = LS, 3 = RO
static uint32_t do_shift(M68000CPU *cpu, uint32_t val, unsigned count, int type,
                         int left, int size) {
    uint32_t mask = size_mask(size), msb = size_msb(size);
    val &= mask;
    if (count == 0) {
...
static int cond_true(const M68000CPU *cpu, int cc) {
...
static uint32_t index_ext(M68000CPU *cpu, uint32_t base) {
...
static int ea_calc(M68000CPU *cpu, int mode, int reg, int size, EA *ea) {
...
static uint32_t ea_read(M68000CPU *cpu, const EA *ea, int size) {
    switch (ea->kind) {
...
static void ea_write(M68000CPU *cpu, const EA *ea, uint32_t val, int size) {
...
static int do_illegal(M68000CPU *cpu) {
    cpu->halted = 1;
    return 1;
}

// MOVEM helper
static int exec_movem(M68000CPU *cpu, uint16_t op) {
    int to_regs = (op >> 10) & 1;
...
int m68000_step(void *context) {
    ...
    switch (op >> 12) {
        case 0x0: { // Immediate group: ORI/ANDI/SUBI/ADDI/EORI/CMPI
...
void m68000_print_state(void *context) {
    ...
    printf("M68000 CPU State (Ticks: %u):\n", cpu->ticks);
...
static uint16_t dis_fetch16(M68000CPU *cpu, uint32_t *p) {
...
static void dis_ea(M68000CPU *cpu, uint32_t *p, int mode, int reg, int size,
                   char *out, size_t n) {
    switch (mode) {
