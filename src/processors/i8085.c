...
// Register indices as encoded in 8085 opcodes: B,C,D,E,H,L,M,A
enum { R_B = 0, R_C, R_D, R_E, R_H, R_L, R_M, R_A };

typedef struct I8085CPU {
    ...
    // 8085 interrupt mask byte, RIM layout:
    // bit 0-2: RST5.5/6.5/7.5 masks, bit 3: IE, bit 4-6: pending, bit 7: SID
    uint8_t int_mask;    // RST5.5/6.5/7.5 masks (bits 0-2)
    uint8_t rst75_latch; // pending RST7.5 flip-flop
    uint8_t sod;         // serial output data latch
} I8085CPU;
...
static void set_rp(I8085CPU *cpu, uint8_t rp, uint16_t val) {
    switch (rp) {
...
static int check_cond(I8085CPU *cpu, uint8_t cond) {
    switch (cond) {
...
static void push16(I8085CPU *cpu, uint16_t val) {
...
static uint16_t pop16(I8085CPU *cpu) {
...
static uint8_t get_psw(I8085CPU *cpu) {
...
static void set_psw(I8085CPU *cpu, uint8_t psw) {
...
static void alu_op(I8085CPU *cpu, uint8_t alu, uint8_t val) {
    ...
    switch (alu) {
        ...
        case 3: { // SBB
            uint16_t sub = (uint16_t)val + cpu->flags_c;
        ...
        case 4: { // ANA
            uint8_t res = acc & val;
            cpu->flags_ac = 1; // 8085 always sets AC on ANA/ANI
            cpu->regs[R_A] = res;
            cpu->flags_c = 0;
...
void* i8085_create(void) {
    return calloc(1, sizeof(I8085CPU));
}
...
int i8085_init(void *context) {
    ...
    cpu->int_mask = 0x07; // all RST interrupts masked at reset
    cpu->rst75_latch = 0;
    cpu->sod = 0;
    return 0;
}
...
static int instr_len(uint8_t op) {
    ...
    // 2-byte: MVI (00ddd110), ALU immediate (11aaa110), IN/OUT
...
int i8085_step(void *context) {
    ...
    switch (op) {
        case 0x00: // NOP
        case 0x08: case 0x10: case 0x18: case 0x28: case 0x38: // undefined on 8085; treated as NOP
            break;

        // --- RIM / SIM (8085 only) ---
        case 0x20: // RIM: A = SID | pending | IE | masks
            cpu->regs[R_A] = (uint8_t)((cpu->int_mask & 0x07)
                           | (cpu->interrupts_enabled ? 0x08 : 0x00)
                           | (cpu->rst75_latch ? 0x40 : 0x00)
                           | (cpu->sod ? 0x80 : 0x00)); // SID modeled from SOD latch
            break;
        case 0x30: // SIM
            if (cpu->regs[R_A] & 0x08) // MSE: load new masks from bits 0-2
                cpu->int_mask = cpu->regs[R_A] & 0x07;
            if (cpu->regs[R_A] & 0x10) // R7.5: reset RST7.5 flip-flop
                cpu->rst75_latch = 0;
            if (cpu->regs[R_A] & 0x40) // SOE: latch serial output data
                cpu->sod = (cpu->regs[R_A] & 0x80) ? 1 : 0;
            break;
        ...
        case 0x17: { // RAL
            uint8_t msb = (cpu->regs[R_A] >> 7) & 1;
            cpu->regs[R_A] = (uint8_t)((cpu->regs[R_A] << 1) | cpu->flags_c);
            cpu->flags_c = msb;
            break;
        }
        case 0x1F: { // RAR
            uint8_t lsb = cpu->regs[R_A] & 1;
            cpu->regs[R_A] = (uint8_t)((cpu->regs[R_A] >> 1) | (cpu->flags_c << 7));
            cpu->flags_c = lsb;
            break;
        }
        ...
        // --- CMA / STC / CMC ---
        case 0x2F: cpu->regs[R_A] = (uint8_t)~cpu->regs[R_A]; break; // CMA
        ...
        case 0xF5: push16(cpu, (uint16_t)(((uint16_t)cpu->regs[R_A] << 8) | get_psw(cpu))); break; // PUSH PSW
        ...
        case 0xF1: { uint16_t v = pop16(cpu); set_psw(cpu, v & 0xFF); cpu->regs[R_A] = (uint8_t)(v >> 8); break; } // POP PSW
...
void i8085_print_state(void *context) {
    ...
    printf("Intel 8085 CPU State (Ticks: %u):\n", cpu->ticks);
    ...
    printf("  IntMask: 0x%02X  IE: %d  RST7.5 pending: %d  SOD: %d\n",
           cpu->int_mask, cpu->interrupts_enabled, cpu->rst75_latch, cpu->sod);
}
...
void i8085_get_disassembly(void *context, char *buf, size_t buf_len) {
    ...
    switch (op) {
        ...
        case 0x20: snprintf(buf, buf_len, "RIM"); break;
        case 0x30: snprintf(buf, buf_len, "SIM"); break;
        case 0x01: case 0x11: case 0x21: case 0x31:
