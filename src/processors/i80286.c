// Intel 80286 CPU core (real mode only).
//
// Implements the full 8086 instruction set, the 186-class additions
// (PUSHA/POPA, PUSH imm, IMUL imm, shift-by-imm8, ENTER/LEAVE, BOUND,
// INS/OUTS) and the 286 system instructions modeled in a simplified way:
// SMSW/LMSW operate on an MSW register, SGDT/SIDT/LGDT/LIDT store/load
// descriptor-table registers, CLTS clears MSW.TS, and LAR/LSL simply clear
// ZF (descriptor not loaded). Protected mode is NOT executed: setting
// MSW.PE via LMSW is recorded but the core keeps running in real mode.
// The address unit is 24-bit capable, but memory is kept at 1 MB.
...
#define MEM_SIZE 0x100000 // 1 MB (24-bit-capable address unit, 1 MB populated)

// MSW bits
#define MSW_PE 0x0001 // Protection Enable (recorded only; not executed)
...
// 16-bit register indices as encoded in x86 opcodes
enum { R_AX = 0, R_CX, R_DX, R_BX, R_SP, R_BP, R_SI, R_DI };
// Segment register indices as encoded in x86 opcodes
enum { SEG_ES = 0, SEG_CS, SEG_SS, SEG_DS };

// Descriptor-table register (GDTR/IDTR): 16-bit limit, 24-bit base
typedef struct DescTable {
    uint16_t limit;
    uint32_t base; // 24-bit
} DescTable;

typedef struct I80286CPU {
    ...
    // 286 system state (modeled, not executed)
...
// --- Memory access (segmented; 24-bit-capable linear, wrapped to 1 MB) ---

static inline uint32_t linear(uint16_t seg, uint16_t off) {
    return (((uint32_t)seg << 4) + off) & 0xFFFFFF;
}

static inline uint8_t mem_read8(I80286CPU *cpu, uint16_t seg, uint16_t off) {
    return cpu->memory[linear(seg, off) & (MEM_SIZE - 1)];
}

static inline void mem_write8(I80286CPU *cpu, uint16_t seg, uint16_t off, uint8_t val) {
    cpu->memory[linear(seg, off) & (MEM_SIZE - 1)] = val;
}

static inline uint16_t mem_read16(I80286CPU *cpu, uint16_t seg, uint16_t off) {
    return (uint16_t)(mem_read8(cpu, seg, off) | ((uint16_t)mem_read8(cpu, seg, (uint16_t)(off + 1)) << 8));
}

static inline void mem_write16(I80286CPU *cpu, uint16_t seg, uint16_t off, uint16_t val) {
    mem_write8(cpu, seg, off, (uint8_t)(val & 0xFF));
    mem_write8(cpu, seg, (uint16_t)(off + 1), (uint8_t)(val >> 8));
}

static inline uint8_t fetch8(I80286CPU *cpu) {
    uint8_t v = mem_read8(cpu, cpu->segs[SEG_CS], cpu->ip);
    cpu->ip++;
    return v;
}

static inline uint16_t fetch16(I80286CPU *cpu) {
...
static inline uint8_t get_reg8(I80286CPU *cpu, uint8_t r) {
    if (r < 4) return (uint8_t)(cpu->regs[r] & 0xFF);
    return (uint8_t)(cpu->regs[r - 4] >> 8);
}

static inline void set_reg8(I80286CPU *cpu, uint8_t r, uint8_t val) {
...
static RMOperand decode_modrm(I80286CPU *cpu, uint8_t modrm, int seg_ovr) {
    RMOperand op;
...
static inline uint8_t read_rm8(I80286CPU *cpu, const RMOperand *op) {
    if (op->is_reg) return get_reg8(cpu, op->reg);
    return mem_read8(cpu, op->seg, op->off);
}

static inline void write_rm8(I80286CPU *cpu, const RMOperand *op, uint8_t val) {
    if (op->is_reg) set_reg8(cpu, op->reg, val);
    else mem_write8(cpu, op->seg, op->off, val);
}

static inline uint16_t read_rm16(I80286CPU *cpu, const RMOperand *op) {
    if (op->is_reg) return cpu->regs[op->reg];
    return mem_read16(cpu, op->seg, op->off);
}

static inline void write_rm16(I80286CPU *cpu, const RMOperand *op, uint16_t val) {
...
static inline void update_flags_zsp8(I80286CPU *cpu, uint8_t res) {
    cpu->flags_z = (res == 0) ? 1 : 0;
    cpu->flags_s = (res & 0x80) ? 1 : 0;
    cpu->flags_p = calculate_parity(res);
}

static inline void update_flags_zsp16(I80286CPU *cpu, uint16_t res) {
    cpu->flags_z = (res == 0) ? 1 : 0;
    cpu->flags_s = (res & 0x8000) ? 1 : 0;
    cpu->flags_p = calculate_parity((uint8_t)(res & 0xFF));
}

static uint8_t add8(I80286CPU *cpu, uint8_t a, uint8_t b, uint8_t carry_in) {
...
static uint16_t add16(I80286CPU *cpu, uint16_t a, uint16_t b, uint8_t carry_in) {
...
static uint8_t sub8(I80286CPU *cpu, uint8_t a, uint8_t b, uint8_t borrow_in) {
...
static uint16_t sub16(I80286CPU *cpu, uint16_t a, uint16_t b, uint8_t borrow_in) {
...
static inline void flags_logic8(I80286CPU *cpu, uint8_t res) {
    cpu->flags_c = 0;
    cpu->flags_o = 0;
    cpu->flags_a = 0;
    update_flags_zsp8(cpu, res);
...
static inline void flags_logic16(I80286CPU *cpu, uint16_t res) {
    cpu->flags_c = 0;
    cpu->flags_o = 0;
    cpu->flags_a = 0;
    update_flags_zsp16(cpu, res);
...
// FLAGS word: CF | 1 | PF | 0 | AF | 0 | ZF | SF | TF | IF | DF | OF
// On the 286 in real mode, bits 12-15 (IOPL/NT) read as 0.
static uint16_t get_flags_word(I80286CPU *cpu) {
    uint16_t f = 0x0002;
...
static void set_flags_word(I80286CPU *cpu, uint16_t f) {
...
static uint8_t alu8(I80286CPU *cpu, uint8_t idx, uint8_t a, uint8_t b) {
    switch (idx) {
...
static uint16_t alu16(I80286CPU *cpu, uint8_t idx, uint16_t a, uint16_t b) {
    switch (idx) {
...
static int check_cond(I80286CPU *cpu, uint8_t cond) {
    int r = 0;
    switch (cond >> 1) {
...
static inline void push16(I80286CPU *cpu, uint16_t val) {
    cpu->regs[R_SP] = (uint16_t)(cpu->regs[R_SP] - 2);
    mem_write16(cpu, cpu->segs[SEG_SS], cpu->regs[R_SP], val);
}

static inline uint16_t pop16(I80286CPU *cpu) {
...
static uint8_t shift8(I80286CPU *cpu, uint8_t idx, uint8_t val, uint8_t count) {
    count &= 0x1F; // the 286 masks shift counts to 5 bits
...
static uint16_t shift16(I80286CPU *cpu, uint8_t idx, uint16_t val, uint8_t count) {
    count &= 0x1F; // the 286 masks shift counts to 5 bits
    if (count == 0) return val;
    for (uint8_t i = 0; i < count; ++i) {
...
// --- I/O ports (no devices attached: reads return 0, writes are discarded) ---

static inline uint8_t io_read8(I80286CPU *cpu, uint16_t port) {
    (void)cpu; (void)port;
    return 0;
}

static inline uint16_t io_read16(I80286CPU *cpu, uint16_t port) {
    (void)cpu; (void)port;
    return 0;
}

static inline void io_write8(I80286CPU *cpu, uint16_t port, uint8_t val) {
    (void)cpu; (void)port; (void)val;
}

static inline void io_write16(I80286CPU *cpu, uint16_t port, uint16_t val) {
    (void)cpu; (void)port; (void)val;
}

// --- 16-bit IMUL with truncation flags (used by IMUL r16, r/m16, imm) ---

static uint16_t imul16(I80286CPU *cpu, uint16_t a, uint16_t b) {
    int32_t prod = (int32_t)(int16_t)a * (int16_t)b;
    uint16_t res = (uint16_t)((uint32_t)prod & 0xFFFF);
    cpu->flags_c = cpu->flags_o = (prod != (int16_t)res) ? 1 : 0;
    return res;
}
...
void* i80286_create(void) {
    return calloc(1, sizeof(I80286CPU));
}
...
int i80286_init(void *context) {
    if (!context) return -1;
    I80286CPU *cpu = (I80286CPU*)context;
    memset(cpu, 0, sizeof(I80286CPU));
    cpu->msw = 0xFFF0;        // 286 reset value: PE/MP/EM/TS clear, high bits set
    cpu->idtr.limit = 0x03FF; // real-mode IVT at physical 0
    return 0;
}
...
// --- 0F-prefixed 286 system instructions (simplified models) ---

static int exec_0f(I80286CPU *cpu, int seg_ovr) {
    uint8_t op2 = fetch8(cpu);
    switch (op2) {
        case 0x00: { // SLDT/STR/LLDT/LTR/VERR/VERW: protected-mode only, modeled as no-ops
            uint8_t modrm = fetch8(cpu);
            (void)decode_modrm(cpu, modrm, seg_ovr);
            break;
        }
        case 0x01: { // SGDT/SIDT/LGDT/LIDT/SMSW/LMSW
            ...
            switch (idx) {
                case 0: case 1: { // SGDT / SIDT m
                    DescTable *dt = (idx == 0) ? &cpu->gdtr : &cpu->idtr;
                    if (!rm.is_reg) {
                        mem_write16(cpu, rm.seg, rm.off, dt->limit);
                        mem_write8(cpu, rm.seg, (uint16_t)(rm.off + 2), (uint8_t)(dt->base & 0xFF));
                        mem_write8(cpu, rm.seg, (uint16_t)(rm.off + 3), (uint8_t)((dt->base >> 8) & 0xFF));
                        mem_write8(cpu, rm.seg, (uint16_t)(rm.off + 4), (uint8_t)((dt->base >> 16) & 0xFF));
                        mem_write8(cpu, rm.seg, (uint16_t)(rm.off + 5), 0xFF); // 286 stores FF in byte 5
                    }
                    break;
                }
                case 2: case 3: { // LGDT / LIDT m
                    DescTable *dt = (idx == 2) ? &cpu->gdtr : &cpu->idtr;
                    if (!rm.is_reg) {
                        dt->limit = mem_read16(cpu, rm.seg, rm.off);
                        dt->base = (uint32_t)mem_read8(cpu, rm.seg, (uint16_t)(rm.off + 2))
                                 | ((uint32_t)mem_read8(cpu, rm.seg, (uint16_t)(rm.off + 3)) << 8)
                                 | ((uint32_t)mem_read8(cpu, rm.seg, (uint16_t)(rm.off + 4)) << 16);
                    }
                    break;
                }
                case 4: // SMSW r/m16
                    write_rm16(cpu, &rm, cpu->msw);
                    break;
                case 6: { // LMSW r/m16: PE can be set but never cleared; not acted upon
                    uint16_t v = read_rm16(cpu, &rm);
                    cpu->msw = (uint16_t)((cpu->msw & MSW_PE) | (v & (MSW_PE | MSW_MP | MSW_EM | MSW_TS)) | 0xFFF0);
                    break;
                }
                default:
                    break;
        ...
        case 0x02: case 0x03: { // LAR / LSL r16, r/m16: no descriptors in real mode -> ZF=0
            uint8_t modrm = fetch8(cpu);
            (void)decode_modrm(cpu, modrm, seg_ovr);
            cpu->flags_z = 0; // descriptor not loaded, destination unchanged
            break;
        }
        case 0x06: // CLTS
            cpu->msw = (uint16_t)(cpu->msw & ~MSW_TS);
            break;
        default:
            // Other 0F opcodes are undefined on the 286; treated as NOP.
            break;
    }
    return 0;
}
...
int i80286_step(void *context) {
    ...
    switch (op) {
        ...
        // --- BOUND r16, m16&16 (186+): out-of-range would raise INT 5 ---
        case 0x62: {
            uint8_t modrm = fetch8(cpu);
            uint8_t reg = (uint8_t)((modrm >> 3) & 7);
            RMOperand rm = decode_modrm(cpu, modrm, seg_ovr);
            if (!rm.is_reg) {
                int16_t idx16 = (int16_t)cpu->regs[reg];
                int16_t lo = (int16_t)mem_read16(cpu, rm.seg, rm.off);
                int16_t hi = (int16_t)mem_read16(cpu, rm.seg, (uint16_t)(rm.off + 2));
                if (idx16 < lo || idx16 > hi) {
                    // No interrupt delivery in this core: treat INT 5 as a fault halt.
                    cpu->halted = 1;
                    return -2;
                }
            }
        ...
        // --- IMUL r16, r/m16, imm16 / imm8 (186+) ---
        case 0x69: case 0x6B: {
            uint8_t modrm = fetch8(cpu);
            uint8_t reg = (uint8_t)((modrm >> 3) & 7);
            RMOperand rm = decode_modrm(cpu, modrm, seg_ovr);
            uint16_t src = read_rm16(cpu, &rm);
            uint16_t imm = (op == 0x69) ? fetch16(cpu) : (uint16_t)(int16_t)(int8_t)fetch8(cpu);
            cpu->regs[reg] = imul16(cpu, src, imm);
            break;
        }

        // --- INS / OUTS (186+, with optional REP) ---
        case 0x6C: case 0x6D: case 0x6E: case 0x6F: {
            int wide = op & 1;
            int is_ins = (op < 0x6E);
            int16_t delta = (int16_t)(cpu->flags_d ? -(wide ? 2 : 1) : (wide ? 2 : 1));
            uint16_t port = cpu->regs[R_DX];
            uint16_t src_seg = cpu->segs[(seg_ovr >= 0) ? seg_ovr : SEG_DS];
            for (;;) {
                if (rep && cpu->regs[R_CX] == 0) break;
                if (is_ins) { // INS: port -> ES:DI
                    if (wide) mem_write16(cpu, cpu->segs[SEG_ES], cpu->regs[R_DI], io_read16(cpu, port));
                    else mem_write8(cpu, cpu->segs[SEG_ES], cpu->regs[R_DI], io_read8(cpu, port));
                    cpu->regs[R_DI] = (uint16_t)(cpu->regs[R_DI] + delta);
                } else {      // OUTS: DS:SI -> port
                    if (wide) io_write16(cpu, port, mem_read16(cpu, src_seg, cpu->regs[R_SI]));
                    else io_write8(cpu, port, mem_read8(cpu, src_seg, cpu->regs[R_SI]));
        ...
        // --- Shift/rotate by imm8 (186+) ---
        case 0xC0: {
            uint8_t modrm = fetch8(cpu);
            RMOperand rm = decode_modrm(cpu, modrm, seg_ovr);
            uint8_t val = read_rm8(cpu, &rm);
            write_rm8(cpu, &rm, shift8(cpu, (uint8_t)((modrm >> 3) & 7), val, fetch8(cpu)));
            break;
        }
        case 0xC1: {
            uint8_t modrm = fetch8(cpu);
            ...
            write_rm16(cpu, &rm, shift16(cpu, (uint8_t)((modrm >> 3) & 7), val, fetch8(cpu)));
            break;
        }
        ...
        case 0xC8: {
            ...
            cpu->regs[R_BP] = frame;
            cpu->regs[R_SP] = (uint16_t)(cpu->regs[R_SP] - size);
            break;
        }
        case 0xC9: // LEAVE
        ...
        // --- IN / OUT ---
        case 0xE4: set_reg8(cpu, 0, io_read8(cpu, fetch8(cpu))); break;
        case 0xE5: cpu->regs[R_AX] = io_read16(cpu, fetch8(cpu)); break;
        case 0xE6: { uint16_t port = fetch8(cpu); io_write8(cpu, port, get_reg8(cpu, 0)); break; }
        case 0xE7: { uint16_t port = fetch8(cpu); io_write16(cpu, port, cpu->regs[R_AX]); break; }
        case 0xEC: set_reg8(cpu, 0, io_read8(cpu, cpu->regs[R_DX])); break;
        case 0xED: cpu->regs[R_AX] = io_read16(cpu, cpu->regs[R_DX]); break;
        case 0xEE: io_write8(cpu, cpu->regs[R_DX], get_reg8(cpu, 0)); break;
        case 0xEF: io_write16(cpu, cpu->regs[R_DX], cpu->regs[R_AX]); break;
...
void i80286_print_state(void *context) {
    ...
    printf("Intel 80286 CPU State (Ticks: %u):\n", cpu->ticks);
    printf("  CS:IP: %04X:%04X (linear 0x%06X)  Halted: %s\n",
    ...
    printf("  MSW: 0x%04X (PE=%d MP=%d EM=%d TS=%d)\n",
           cpu->msw, (cpu->msw & MSW_PE) ? 1 : 0, (cpu->msw & MSW_MP) ? 1 : 0,
           (cpu->msw & MSW_EM) ? 1 : 0, (cpu->msw & MSW_TS) ? 1 : 0);
    printf("  GDTR: base=0x%06X limit=0x%04X  IDTR: base=0x%06X limit=0x%04X\n",
           cpu->gdtr.base, cpu->gdtr.limit, cpu->idtr.base, cpu->idtr.limit);
}

// --- Disassembly ---

static uint8_t dpeek8(I80286CPU *cpu, uint16_t *off) {
    uint8_t v = mem_read8(cpu, cpu->segs[SEG_CS], *off);
    (*off)++;
    return v;
}

static uint16_t dpeek16(I80286CPU *cpu, uint16_t *off) {
...
static void dasm_rm(I80286CPU *cpu, uint16_t *off, uint8_t modrm, int wide, char *out, size_t n) {
    uint8_t mod = (uint8_t)(modrm >> 6);
    uint8_t rm = (uint8_t)(modrm & 7);
...
// Disassembles the 0F-prefixed 286 system instructions.
static void dasm_0f(I80286CPU *cpu, uint16_t *off, const char *prefix, char *buf, size_t buf_len) {
    uint8_t op2 = dpeek8(cpu, off);
    char rm_str[32];
    switch (op2) {
        case 0x00: {
            const char* grp_names[] = { "SLDT", "STR", "LLDT", "LTR", "VERR", "VERW", "???", "???" };
        ...
        case 0x01: {
            const char* grp_names[] = { "SGDT", "SIDT", "LGDT", "LIDT", "SMSW", "???", "LMSW", "???" };
            uint8_t modrm = dpeek8(cpu, off);
            dasm_rm(cpu, off, modrm, 1, rm_str, sizeof(rm_str));
            snprintf(buf, buf_len, "%s%-5s %s", prefix, grp_names[(modrm >> 3) & 7], rm_str);
            return;
        ...
        case 0x02: case 0x03: {
            uint8_t modrm = dpeek8(cpu, off);
            dasm_rm(cpu, off, modrm, 1, rm_str, sizeof(rm_str));
            snprintf(buf, buf_len, "%s%-5s %s, %s", prefix, (op2 == 0x02) ? "LAR" : "LSL",
                     reg16_names[(modrm >> 3) & 7], rm_str);
            return;
        }
        case 0x06: snprintf(buf, buf_len, "CLTS"); return;
        default:
            snprintf(buf, buf_len, "DB    0x0F, 0x%02X", op2);
            return;
    }
}
...
void i80286_get_disassembly(void *context, char *buf, size_t buf_len) {
    ...
    // 0F: 286 system instructions
    if (op == 0x0F) {
        dasm_0f(cpu, &off, prefix, buf, buf_len);
        return;
    }
    ...
    switch (op) {
        ...
        case 0xC0: case 0xC1: {
            uint8_t modrm = dpeek8(cpu, &off);
            dasm_rm(cpu, &off, modrm, op & 1, rm_str, sizeof(rm_str));
            snprintf(buf, buf_len, "%s%-5s %s, %u", prefix, shift_names[(modrm >> 3) & 7], rm_str, dpeek8(cpu, &off));
            return;
        }
        ...
        case 0xC8: {
            uint16_t size = dpeek16(cpu, &off);
            snprintf(buf, buf_len, "ENTER 0x%04X, %u", size, dpeek8(cpu, &off));
            return;
        }
        ...
        case 0xE4: snprintf(buf, buf_len, "IN    AL, 0x%02X", dpeek8(cpu, &off)); return;
        case 0xE5: snprintf(buf, buf_len, "IN    AX, 0x%02X", dpeek8(cpu, &off)); return;
        case 0xE6: snprintf(buf, buf_len, "OUT   0x%02X, AL", dpeek8(cpu, &off)); return;
        case 0xE7: snprintf(buf, buf_len, "OUT   0x%02X, AX", dpeek8(cpu, &off)); return;
        case 0xEC: snprintf(buf, buf_len, "IN    AL, DX"); return;
        case 0xED: snprintf(buf, buf_len, "IN    AX, DX"); return;
        case 0xEE: snprintf(buf, buf_len, "OUT   DX, AL"); return;
        case 0xEF: snprintf(buf, buf_len, "OUT   DX, AX"); return;
        case 0xE8: {
