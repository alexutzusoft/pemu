...
static inline uint8_t mem_read8(I8086CPU *cpu, uint16_t seg, uint16_t off) {
    return cpu->memory[linear(seg, off)];
}

static inline void mem_write8(I8086CPU *cpu, uint16_t seg, uint16_t off, uint8_t val) {
    cpu->memory[linear(seg, off)] = val;
}

static inline uint16_t mem_read16(I8086CPU *cpu, uint16_t seg, uint16_t off) {
    return (uint16_t)(mem_read8(cpu, seg, off) | ((uint16_t)mem_read8(cpu, seg, (uint16_t)(off + 1)) << 8));
}

static inline void mem_write16(I8086CPU *cpu, uint16_t seg, uint16_t off, uint16_t val) {
    mem_write8(cpu, seg, off, (uint8_t)(val & 0xFF));
    mem_write8(cpu, seg, (uint16_t)(off + 1), (uint8_t)(val >> 8));
}

static inline uint8_t fetch8(I8086CPU *cpu) {
    uint8_t v = mem_read8(cpu, cpu->segs[SEG_CS], cpu->ip);
    cpu->ip++;
    return v;
}

static inline uint16_t fetch16(I8086CPU *cpu) {
...
static inline uint8_t get_reg8(I8086CPU *cpu, uint8_t r) {
    if (r < 4) return (uint8_t)(cpu->regs[r] & 0xFF);
    return (uint8_t)(cpu->regs[r - 4] >> 8);
}

static inline void set_reg8(I8086CPU *cpu, uint8_t r, uint8_t val) {
...
static RMOperand decode_modrm(I8086CPU *cpu, uint8_t modrm, int seg_ovr) {
    RMOperand op;
...
static inline uint8_t read_rm8(I8086CPU *cpu, const RMOperand *op) {
    if (op->is_reg) return get_reg8(cpu, op->reg);
    return mem_read8(cpu, op->seg, op->off);
}

static inline void write_rm8(I8086CPU *cpu, const RMOperand *op, uint8_t val) {
    if (op->is_reg) set_reg8(cpu, op->reg, val);
    else mem_write8(cpu, op->seg, op->off, val);
}

static inline uint16_t read_rm16(I8086CPU *cpu, const RMOperand *op) {
    if (op->is_reg) return cpu->regs[op->reg];
    return mem_read16(cpu, op->seg, op->off);
}

static inline void write_rm16(I8086CPU *cpu, const RMOperand *op, uint16_t val) {
...
static inline void update_flags_zsp8(I8086CPU *cpu, uint8_t res) {
    cpu->flags_z = (res == 0) ? 1 : 0;
    cpu->flags_s = (res & 0x80) ? 1 : 0;
    cpu->flags_p = calculate_parity(res);
}

static inline void update_flags_zsp16(I8086CPU *cpu, uint16_t res) {
    cpu->flags_z = (res == 0) ? 1 : 0;
    cpu->flags_s = (res & 0x8000) ? 1 : 0;
    cpu->flags_p = calculate_parity((uint8_t)(res & 0xFF));
}

static uint8_t add8(I8086CPU *cpu, uint8_t a, uint8_t b, uint8_t carry_in) {
...
static uint16_t add16(I8086CPU *cpu, uint16_t a, uint16_t b, uint8_t carry_in) {
...
static uint8_t sub8(I8086CPU *cpu, uint8_t a, uint8_t b, uint8_t borrow_in) {
...
static uint16_t sub16(I8086CPU *cpu, uint16_t a, uint16_t b, uint8_t borrow_in) {
...
static inline void flags_logic8(I8086CPU *cpu, uint8_t res) {
    cpu->flags_c = 0;
    cpu->flags_o = 0;
    cpu->flags_a = 0;
    update_flags_zsp8(cpu, res);
...
static inline void flags_logic16(I8086CPU *cpu, uint16_t res) {
    cpu->flags_c = 0;
    cpu->flags_o = 0;
    cpu->flags_a = 0;
    update_flags_zsp16(cpu, res);
...
static uint16_t get_flags_word(I8086CPU *cpu) {
    uint16_t f = 0xF002;
...
static void set_flags_word(I8086CPU *cpu, uint16_t f) {
...
static uint8_t alu8(I8086CPU *cpu, uint8_t idx, uint8_t a, uint8_t b) {
    switch (idx) {
...
static uint16_t alu16(I8086CPU *cpu, uint8_t idx, uint16_t a, uint16_t b) {
    switch (idx) {
...
static int check_cond(I8086CPU *cpu, uint8_t cond) {
    int r = 0;
    switch (cond >> 1) {
...
static inline void push16(I8086CPU *cpu, uint16_t val) {
    cpu->regs[R_SP] = (uint16_t)(cpu->regs[R_SP] - 2);
    mem_write16(cpu, cpu->segs[SEG_SS], cpu->regs[R_SP], val);
}

static inline uint16_t pop16(I8086CPU *cpu) {
...
static uint8_t shift8(I8086CPU *cpu, uint8_t idx, uint8_t val, uint8_t count) {
    count &= 0x1F;
...
static uint16_t shift16(I8086CPU *cpu, uint8_t idx, uint16_t val, uint8_t count) {
    count &= 0x1F;
    if (count == 0) return val;
    for (uint8_t i = 0; i < count; ++i) {
...
void* i8086_create(void) {
    return calloc(1, sizeof(I8086CPU));
}
...
void i8086_print_state(void *context) {
    ...
    printf("Intel 8086 CPU State (Ticks: %u):\n", cpu->ticks);
...
static uint8_t dpeek8(I8086CPU *cpu, uint16_t *off) {
    uint8_t v = mem_read8(cpu, cpu->segs[SEG_CS], *off);
    (*off)++;
    return v;
}

static uint16_t dpeek16(I8086CPU *cpu, uint16_t *off) {
...
static void dasm_rm(I8086CPU *cpu, uint16_t *off, uint8_t modrm, int wide, char *out, size_t n) {
    uint8_t mod = (uint8_t)(modrm >> 6);
    uint8_t rm = (uint8_t)(modrm & 7);
